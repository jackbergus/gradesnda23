...
def minerful_run(jar_path, minL, maxL, logS, logName, wd, csv, model_size, timeout=None):
    ...
    try:
        ...
        os.chdir(cwd)
        csv.writerow([logName, minL, maxL, logS, model_size, 'minerful', timeDelta])
        ...
        breakMe=True
    os.chdir(cwd)
    # L = output.splitlines()[2:]
    # logScan = {tuple(x[1:-1].split(",")) for x in L}
    # print(str(len(logScan))+" vs "+str(len(L)))
